...
class CollegeOne extends Component {
	...
	componentWillMount() {
		collegeCourse('college_one').then((data) => {
			this.setState({
				courseList: data
			})
		}, (rejected) => {
		...
		return (
			<div>
				<div className='tab_wrap'>
				    <div className='tab'>
				    	<a href='#/collegeOne' className='tab_a'>大一课程<span className='triangle'></span></a>
				    	<a href='#/collegeTwo' >大二课程</a>
				    	<a href='#/collegeThree' >大三课程</a>
						...
						return(
							<div className='col-sm-4 col-md-3 col-xs-6' key={index}>
									...
									<figure className='course_item '>
									    <a href={'#/couserDetail/'+item._id} onClick={()=>_this.seeFile(item._id)} className='tab_href'></a>
